...
const char *vertex_shader_path = "assets/shaders/shader.vert";
const char *fragment_shader_path = "assets/shaders/shader.frag";

// camera
Camera camera(glm::vec3(0.0f, 0.0f, 3.0f));
float last_x = (float)SCR_WIDTH / 2.0f;
float last_y = (float)SCR_HEIGHT / 2.0f;
...
int main()
{
    ...
    unsigned char *data = stbi_load(texture1_path, &width, &height, &nr_channels, 0);
    if (data)
    {
    ...
    data = stbi_load(texture2_path, &width, &height, &nr_channels, 0);
    if (data)
    {
